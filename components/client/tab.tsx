...
const Tab: FC<TabProps> = ({ selectedTab, settingsOption, setter }) => {
    return <div className="w-full flex gap-x-3 my-5">
        {settingsOption.map((option, index) => (
            <span className={`block px-5 py-2 ${selectedTab == option.name ? 'bg-darkerMain text-white' : 'bg-zinc-50 text-slate-700  hover:bg-zinc-200/50'} rounded-full font-medium cursor-pointer transition-300 capitalize`} key={index} onClick={() => setter(option.name)}>
                {option.name}
            </span>
        ))}
    </div>
