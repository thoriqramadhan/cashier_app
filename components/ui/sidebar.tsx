...
const SidebarItem: FC<SidebarItemProps> = ({ sidebarValue, pathNow, isAdmin }) => {
    ...
    return <Link className={cn(sidebarValue.admin && 'hidden', sidebarValue.admin && isAdmin && 'block')} href={`/${sidebarValue.name}`} onClick={() => setter('setPath', sidebarValue.name as pathOption)}>
